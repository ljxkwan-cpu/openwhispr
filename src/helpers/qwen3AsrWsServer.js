...
class Qwen3AsrWsServer {
  ...
  async _doStart(modelName, modelDir) {
    ...
    const args = [
      `--qwen3-asr-conv-frontend=${path.join(modelDir, "conv_frontend.onnx")}`,
      `--qwen3-asr-encoder=${path.join(modelDir, "encoder.int8.onnx")}`,
      `--qwen3-asr-decoder=${path.join(modelDir, "decoder.int8.onnx")}`,
      `--qwen3-asr-tokenizer=${path.join(modelDir, "tokenizer")}`,
      `--qwen3-asr-max-new-tokens=512`,
      `--port=${this.port}`,
      `--num-threads=${Math.max(1, Math.min(4, Math.floor(os.cpus().length * 0.75)))}`,
    ];

    debugLogger.debug("Starting qwen3-asr WS server", { port: this.port, modelName, args });
    ...
    this.process.stdout.on("data", (data) => {
      debugLogger.debug("qwen3-asr-ws stdout", { data: data.toString().trim() });
    });

    this.process.stderr.on("data", (data) => {
      stderrBuffer += data.toString();
      debugLogger.debug("qwen3-asr-ws stderr", { data: data.toString().trim() });
    ...
    this.process.on("error", (error) => {
      debugLogger.error("qwen3-asr-ws process error", { error: error.message });
      this.ready = false;
      readyResolve(false);
    });
    ...
    this.process.on("close", (code) => {
      exitCode = code;
      debugLogger.debug("qwen3-asr-ws process exited", { code });
      this.ready = false;
      this.process = null;
    ...
    debugLogger.info("qwen3-asr-ws server started successfully", {
      port: this.port,
      model: modelName,
    });
  ...
  async _warmUp() {
    try {
      ...
      debugLogger.debug("qwen3-asr-ws warm-up inference complete");
    } catch (err) {
      debugLogger.warn("qwen3-asr-ws warm-up failed (non-fatal)", { error: err.message });
    }
  }
  ...
  async _waitForReady(readySignal, getProcessInfo) {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error(`qwen3-asr-ws failed to start within ${STARTUP_TIMEOUT_MS}ms`)),
        STARTUP_TIMEOUT_MS
      );
    });
    ...
    if (!ready) {
      ...
      throw new Error(`qwen3-asr-ws process died during startup${details ? `: ${details}` : ""}`);
    }

    this.ready = true;
    debugLogger.debug("qwen3-asr-ws ready", { startupTimeMs: Date.now() - startTime });
  }
  ...
  _startHealthCheck() {
    ...
    this.healthCheckInterval = setInterval(() => {
      ...
      if (!this._isProcessAlive()) {
        debugLogger.warn("qwen3-asr-ws health check failed: process not alive");
        this.ready = false;
        this.stopHealthCheck();
      }
  ...
  transcribe(samplesBuffer, sampleRate) {
    if (!this.ready || !this.process) {
      throw new Error("qwen3-asr-ws server is not running");
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        done(reject)(new Error("qwen3-asr-ws transcription timed out"));
      ...
      ws.on("open", () => {
        ...
        debugLogger.debug("qwen3-asr-ws sending audio", {
        ...
        ws.send(message, (err) => {
          if (err) {
            debugLogger.error("qwen3-asr-ws send error", { error: err.message });
          }
        });
      });
      ...
      ws.on("close", (code) => {
        ...
        debugLogger.debug("qwen3-asr-ws transcription completed", {
          elapsed,
          code,
      ...
      ws.on("error", (error) => {
        clearTimeout(timeout);
        done(reject)(new Error(`qwen3-asr-ws transcription failed: ${error.message}`));
      });
    });
  }
  ...
  async stop() {
    ...
    debugLogger.debug("Stopping qwen3-asr-ws server");

    try {
      await gracefulStopProcess(this.process);
    } catch (error) {
      debugLogger.error("Error stopping qwen3-asr-ws server", { error: error.message });
    }
...
module.exports = Qwen3AsrWsServer;
